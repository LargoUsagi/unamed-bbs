...
//!
//! Both links are structured identically: a socket held open for the program
//! lifetime, a buffered writer used from the main thread, and a background
...
pub const Core = struct {
    ...
    pub fn writerInterface(self: *Core) ?*std.Io.Writer {
        return if (self.writer) |*w| &w.interface else null;
    }
    ...
    /// Background reader thread: reads bytes from the stream and hands
    /// them to the link dispatcher until the stream errors, closes, or
    /// `stop` is set.
